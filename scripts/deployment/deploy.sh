...
scp -o StrictHostKeyChecking=no -r -i ${DEPLOY_MACHINE_KEY_FILE} eums/build/*.sh ${DEPLOY_USER}@${DEPLOY_HOST}:~/deploy_$today/scripts
scp -o StrictHostKeyChecking=no -r -i ${DEPLOY_MACHINE_KEY_FILE} contacts/build/*.sh ${DEPLOY_USER}@${DEPLOY_HOST}:~/deploy_$today/scripts
scp -o StrictHostKeyChecking=no -r -i ${DEPLOY_MACHINE_KEY_FILE} eums/build/*.tar ${DEPLOY_USER}@${DEPLOY_HOST}:~/deploy_$today/
scp -o StrictHostKeyChecking=no -r -i ${DEPLOY_MACHINE_KEY_FILE} contacts/build/*.tar ${DEPLOY_USER}@${DEPLOY_HOST}:~/deploy_$today/
