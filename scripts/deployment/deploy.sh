...
sshpass -p "${DEPLOY_USER_PASSWORD}" ssh ${DEPLOY_USER}@${DEPLOY_HOST} "cd /home/${DEPLOY_USER}/ && sudo ./unpack_deployment_and_install.sh $today ${CONTACTS_HOST} ${RAPIDPRO_API_TOKEN} ${EMAIL_PASSWORD} ${VISION_USER} ${VISION_PASSWORD}"
