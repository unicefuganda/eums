...
class SalesOrderEndPointTest(AuthenticatedAPITestCase):
    ...
    def test_should_get_sales_orders_without_release_orders(self):
        ...
        get_response = self.client.get(ENDPOINT_URL+'?with_release_orders=false')

        self.assertEqual(get_response.status_code, 200)
        self.assertDictContainsSubset(created_sales_order, get_response.data[0])

    def test_should_get_sales_orders_with_release_orders(self):
        created_release_order = create_release_order(self)

        get_response = self.client.get(ENDPOINT_URL+'?with_release_orders=true')

        self.assertEqual(get_response.status_code, 200)
        self.assertEqual(created_release_order['id'], get_response.data[0]['release_orders'][0])
