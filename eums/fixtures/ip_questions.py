from eums.models import MultipleChoiceQuestion, Option, TextQuestion, Flow


def setup_ip_flow_questions(flow):
    ip_question_1, _ = MultipleChoiceQuestion.objects.get_or_create(
        ...
        text='Was delivery received?', label='deliveryReceived', flow=flow, position=1)
    ...
    flow.end_nodes = []
    flow.end_nodes.append([ip_question_1.id, no_delivery.id])
    flow.save()

    ip_question_2, _ = TextQuestion.objects.get_or_create(
        uuids=['0f49db8d-432e-4d18-b596-408a0bb2eaa8'],
        text='When was delivery received?', label='dateOfReceipt', flow=flow, position=2)
    ...
    ip_question_5, _ = TextQuestion.objects.get_or_create(
        ...
        text='Additional Remarks', label='additionalDeliveryComments', flow=flow, position=5)
    flow.end_nodes.append([ip_question_5.id, Flow.NO_OPTION])
    flow.save()
